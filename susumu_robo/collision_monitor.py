    ...
    def handle_forward_obstacle_detection(self, pub_msg):
        ...
        for sensor_id in front_sensor_ids:
            if self.range_list[sensor_id] < STOP_THRESHOLD:
                ...
                self.get_logger().info(f"障害物を検知しました. 停止します. センサーid:{sensor_id} range:{self.range_list[sensor_id]}")
                ...
                self.get_logger().info(f"障害物を検知しました. 速度を制限します. センサーid:{sensor_id} range:{self.range_list[sensor_id]}")
