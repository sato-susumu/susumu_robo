"""実機用 launch ファイル（デバッグモード）。

config.yaml の robot.mode を "real" に設定してから使用する。

使い方:
    ros2 launch susumu_robo agent_option_debug.launch.py
    ros2 launch susumu_robo agent_option_debug.launch.py config_path:=/path/to/config.yaml
    ros2 launch susumu_robo agent_option_debug.launch.py cmd_vel_stamped:=true
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        cmd_vel_stamped_arg,
        debug_dir_arg,
        LogInfo(msg="susumu_agent を起動します（デバッグモード）..."),
        agent_node,
    ])
